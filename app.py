...
def normalizar_columnas_y_fecha(df, es_decreto=False):
    """Limpia nombres de columnas, elimina espacios y asegura formato de fecha"""
...
# Historial limitado estrictamente al año 2025 en adelante
ano_inicio_historial = st.sidebar.selectbox("Ver datos históricos desde:", [2025], index=0)
...
# --- CONTROLES DE SIMULACIÓN DE ESCENARIOS FUTUROS ---
st.sidebar.markdown("---")
...
val_sim_emergencias = st.sidebar.slider("Cantidad de Bloqueos/Alertas viales", min_value=0, max_value=5, value=0, step=1)

# ==============================================================================
# FUNCIÓN AUXILIAR: DETERMINAR ESTACIONES EN PERÚ
# ==============================================================================
def obtener_color_estacion_peru(fecha):
    """Devuelve un color de fondo tenue según la estación climática en Perú"""
    mes = fecha.month
    dia = fecha.day
    ...
    if (mes == 12 and dia >= 21) or (mes in [1, 2]) or (mes == 3 and dia <= 20):
        ...
        return "rgba(218, 165, 32, 0.08)"
    elif (mes == 6 and dia >= 22) or (mes in [7, 8]) or (mes == 9 and dia <= 22):
        return "rgba(30, 144, 255, 0.08)"
    else:
        return "rgba(46, 139, 87, 0.08)"
...
def entrenar_y_predecir_estacional(producto, ano_historial, mes_nombre, ano_dest, sim_diesel, sim_emergencias):
    try:
        ...
        # RECONSTRUCCIÓN CRONOLÓGICA (Limpieza de Gaps)
        ...
        # Procesamiento homogéneo de emergencias viales
        emergencias = []
        ...
        # Solución al NAN: Calcular promedio histórico ignorando posibles nulos remanentes
        diesel_medio_hist = df_merge['diesel_lima_soles'].dropna().mean()
        if pd.isna(diesel_medio_hist) or diesel_medio_hist == 0:
            diesel_medio_hist = 17.5  # Respaldo en caso de error de lectura
        ...
        for _ in range(dias_a_predecir):
            ...
            # --- AJUSTE DE SENSIBILIDAD DINÁMICA SIN NANs ---
...
# ==============================================================================
# INTERFAZ Y GRÁFICO AVANZADO (PLOTLY) - DISEÑO ORIGINAL PRESERVADO
# ==============================================================================
with st.spinner("Procesando datos bajo normativas 2025 y estacionalidad de Perú..."):
...
if error:
    ...
    # Mostrar métricas resumidas (Diseño original de dos columnas)
    ...
    # Trazado del gráfico Plotly
    fig = go.Figure()

    # 1. Trazado Línea Histórica (Real) desde 2025
    ...
    # 2. Trazado Línea de Predicción (IA)
    ...
    # FACTORES: Resaltar los picos más altos históricos correlacionados
    ...
    # COLORES POR TEMPORADA PERÚ: Bandas de fondo estructuradas
    ...
    fig.update_layout(
        title=f"Análisis Histórico Integrado (Desde 2025) y Proyección de {producto_seleccionado}",
        xaxis_title="Línea de Tiempo",
        yaxis_title="Soles por Kilograma (S/.)",
    ...
    st.markdown("<small>🍂 **Colores de Fondo (Estaciones Perú):** Rojo Claro = Verano | Amarillo/Marrón = Otoño | Azul Claro = Invierno | Verde Claro = Primavera</small>", unsafe_allow_html=True)